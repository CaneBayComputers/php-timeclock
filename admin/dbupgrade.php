...
if (!empty($count)) {
    ...
    if ($request == 'GET') {
        ...
        if (empty($rows)) {
            $passwd_query = mysqli_query($db, "ALTER TABLE " . $db_prefix . "employees ADD $field VARCHAR(25) NOT NULL;");
        ...
        if (empty($rows)) {
            $passwd_query = mysqli_query($db, "ALTER TABLE " . $db_prefix . "employees ADD $field VARCHAR(75) NOT NULL;");
            echo "              <tr><td width=10 class=table_rows style='padding-left:25px;color:#FF9900;font-weight:bold;'>Added</td><td class=table_rows
                      align=left>:&nbsp;<b>$field</b> field has been added to the <u>employees</u> table.</td></tr>\n";
            $passed_or_not = "1";
        }
        ...
        $result = mysqli_query($db, "SHOW FIELDS FROM " . $db_prefix . "employees");
        while ($row = mysqli_fetch_array($result)) {
            $name = "" . $row["Field"] . "";
            $type = "" . $row["Type"] . "";
            ...
            if (($name == 'empfullname') && ($type != 'varchar(50)')) {
                $alter_result = mysqli_query($db, "ALTER TABLE " . $db_prefix . "employees CHANGE empfullname empfullname VARCHAR(50) NOT NULL");
